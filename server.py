...
class TTT:
    ...
    def game_status_checker(self):
        ...
        if self.board[0][0] == self.board[1][1] == self.board[2][2] and self.board[0][1] != " ":
    ...
    def hoster(self):
        ...
        while True:
            if self.game_status_checker() == True:
                if self.turn == "Fishie":
                    client.send("Game Over! You won Fishie! Congrads! :D".encode('UTF-8'))
                    break
                else:
                    client.send("Game Over! Oppps! Fishie won!".encode('UTF-8'))
                    break
            elif self.game_status_checker() == 'Draw':
                client.send("Game Over! Damn.. It's a Draw. Bro u need to train more....".encode('UTF-8'))
                break
